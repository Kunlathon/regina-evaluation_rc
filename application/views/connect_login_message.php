...
<?php
	...
	if($student_id=="" and $student_Identification==""){
		$txt_error="null";
		exit("<script>window.location='$golink/rc/loginrc?not_null=$not_null';</script>");
...
//data...	
	/*	if($data_status==1){
			$txt_error="key";
			exit("<script>window.location='$golink/rc/loginrc?txt_error=$txt_error';</script>");
		}else{*/
			...
			if($update_loginCr=="yes"){
				...
				//session_write_close();
				$this->session->set_userdata("rc_user",$data_user);
//-----------------------------------------------------------------------------------------------------------------------------------				
