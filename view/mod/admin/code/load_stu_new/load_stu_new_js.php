...
<!-- /theme JS files -->
	...
	$(document).ready(function () {	
		$('#button_run').on('click', function() {
			swal({
				...
				text: "คุณต้องการโหลดนักเรียนใหม่จากระบบรับสมัครนักเรียน",
				type: "warning",
				showCancelButton: true,
				confirmButtonColor: "#FF7043",
				confirmButtonText: "ใช้ ต้องการโหลดข้อมูล"
			},function(buttonrun){
				if(buttonrun){
					var data_year=$("#stu_year").val();
					if(data_year!=""){
						$.post("view/mod/admin/code/load_stu_new/load_stu.php",{
							txt_year:data_year
						},function(datarc){
							if(datarc!=""){
	...
	$(document).ready(function () {
		$('#cp_csc').on('click', function(cpcsc){
			if(cpcsc!=""){
				document.location="./?evaluation_mod=load_stu_new";
			}else{
				//----------------------
			}			
		})
