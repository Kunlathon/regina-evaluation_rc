...
<!-- /theme JS files -->
	...
	$(document).ready(function () {	
		$('#button_run').on('click', function() {
			swal({
				...
				text: "คุณต้องการเลือนปีการศึกษาใช้หรือไม่",
				type: "warning",
				showCancelButton: true,
				confirmButtonColor: "#FF7043",
				confirmButtonText: "ใช้ ต้องการเลือนปีการศึกษา"
			},function(buttonrun){
				if(buttonrun){
					var data_year=$("#stu_year").val();
					if(data_year!=""){
						$.post("view/mod/admin/code/copy_class_year/load_class_year.php",{
							txt_year:data_year
						},function(datarc){
							if(datarc!=""){
	...
	$(document).ready(function () {
		$('#cp_csc').on('click', function(cpcsc){
			if(cpcsc!=""){
				document.location="./?evaluation_mod=copy_class_year";
			}else{
				//----------------------
			}			
		})
